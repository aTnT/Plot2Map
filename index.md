...
Plot2Map is an R package for comparing forest plot data to biomass maps,
with a focus on Above Ground Biomass (AGB) estimation and validation.
...
- Pre-processing various types of plot data (point data, polygons,
  tree-level measurements, lidar)
...
We now preprocess the sampled plot data. The first step is to remove
plots that have been deforested based on the Global Forest Change (GFC)
dataset (Hansen et al., 2013). We use
[`Deforested()`](https://atnt.github.io/Plot2Map/reference/Deforested.md)
that processes each plot, downloads the necessary GFC forest loss tiles,
and determines if the plot has been deforested beyond a set
deforestation threshold or if the deforestation occurred before or
during the specified map year. In the example below we use the default
5% deforestation threshold, and we will validate an AGB map from 2022:
...
- `$all_plots`: The original input `sf` object with added deforestated
  proportion (0-1) w.r.t. to plot area and deforestation start year.

In the next step we assign ecological zones and continents to plot
locations. We use
[`BiomePair()`](https://atnt.github.io/Plot2Map/reference/BiomePair.md)
for this. This function overlays plot locations with pre-processed data
to assign corresponding FAO ecological zones (biomes), Global Ecological
Zones (GEZ) and continents (zones) to each plot:
...
In the step below we use
[`TempApplyVar()`](https://atnt.github.io/Plot2Map/reference/TempApplyVar.md)
to adjust plot biomass values to align with the map year by adding or
subtracting annual growth increment to older or newer plots. The
function uses growth data from a model-based estimate of growth-age
relationships. The function also computes the temporal variance
(standard deviation) of biomass changes based on growth-rate standard
deviations. In the example below we apply temporal adjustment and
calculate variance to all Global Ecological Zones (GEZ) within the plot
data:
...
Note how the `$AGB_T_HA`, `$AGB_T_HA_ORIG` and `$sdGrowth` columns are
added to the plot dataset. The first corresponding to the adjusted
biomass values for the specified `map_year`, the second keeping the
original biomass values before adjustment and the last adding the
temporal standard deviation of the biomass adjustment.

### AGB map validation

Now let’s suppose we want to validate an AGB map on the
`preprocessed_plots_biome_temp` data above. We give an example of doing
this with the
[`invDasymetry()`](https://atnt.github.io/Plot2Map/reference/invDasymetry.md)
function where the AGB map is the ESA-CCI AGB dataset.
...
calculates tree cover fraction and AGB for each plot or cell. In our
case we use a tree cover threshold of 10% meaning that only areas with
tree cover percentage above this threshold will contribute to biomass
estimates.
...
Now that we have the validation data from
[`invDasymetry()`](https://atnt.github.io/Plot2Map/reference/invDasymetry.md),
we can analyze and visualize the results. The function returns a data
frame with several columns, including:

- `plotAGB_[threshold]`: AGB values adjusted for forest cover threshold
  (e.g., plotAGB_10)
...
We can now visualize the relationship between the reference plot data
and map values using the plotting functions:
...
![binned_fig](reference/figures/binned_plot_readme.png)
...
![scatter_fig](reference/figures/scatter_plot_readme.png)
...
function provides an assessment of map accuracy including binned
statistics by AGB ranges.

### Handling Uncertainty

Plot2Map also provides a tool for calculating and incorporating plot
uncertainty in plot data. The
[`calculateTotalUncertainty()`](https://atnt.github.io/Plot2Map/reference/calculateTotalUncertainty.md)
function combines measurement, sampling, and growth uncertainties. When
uncertainty is not provided to plot data in the form of a `varPlot`
column during inverse dasymetric mapping,
[`invDasymetry()`](https://atnt.github.io/Plot2Map/reference/invDasymetry.md)
will call
[`calculateTotalUncertainty()`](https://atnt.github.io/Plot2Map/reference/calculateTotalUncertainty.md)
to calculate and add plot uncertainty to plot data automatically.
...
Plot2Map allows for spatial aggregation of plots into larger cells for
...
1.  **Point data with AGB estimates**:
    ...
    - Unformatted (can be processed with
      [`RawPlots()`](https://atnt.github.io/Plot2Map/reference/RawPlots.md))
2.  **Polygon data with corner coordinates**:
    - Can be processed with
      [`Polygonize()`](https://atnt.github.io/Plot2Map/reference/Polygonize.md)
3.  **Tree-level measurement data**:
    - Processed with `MeasurementErr()` which estimates AGB and
      uncertainty
4.  **Nested plot data (with sub-plots)**:
    - Processed with
      [`Nested()`](https://atnt.github.io/Plot2Map/reference/Nested.md)
      followed by `MeasurementErr()`
...
Araza et al., A comprehensive framework for assessing the accuracy and
uncertainty of global above-ground biomass maps, Remote Sensing of
Environment, Volume 272, 2022, 112917, ISSN 0034-4257,
